...
// Reads the real skeleton assets directly from Data/Skeletons instead of
// copying/symlinking them into a test bundle. (Nanosaur's terrain files are
// .ter/.trt, not resource forks, so only the .rsrc skeletons are fixtures.)
private func fixtureURLs() -> [URL] {
    ...
    guard let enumerator = fm.enumerator(at: dataDir, includingPropertiesForKeys: nil) else { return [] }
    return enumerator
        .compactMap { $0 as? URL }
        .filter { $0.pathExtension == "rsrc" }
...
@Test func deinonSkeletonHasExpectedResources() throws {
    let url = fixtureURLs().first { $0.lastPathComponent == "Deinon.skeleton.rsrc" }!
    let data = try Data(contentsOf: url)
    let file = try ResourceFile(parsing: data)
