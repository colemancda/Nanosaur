...
// (the .skeleton.rsrc files under Data/Skeletons), replacing Pomme's
...
// ThrownParsingError aliases the concrete ParsingError type (with no
// user-error payload) under Embedded Swift, so ResourceFileParsingError's
// specific cases can't be carried through there - they downgrade to a plain
// `.invalidValue` status.
#if $Embedded
